...
if (!publicRelease.repository?.includes('github.com/liufeng1976/bossai-ecommerce-ai-team-skill')) failures.push('public-release.json repository must use the canonical GitHub repository');
...
if (agentApi.bossaiConnector?.automaticExternalActions !== false) failures.push('agent-api.json must deny automatic external actions at the connector boundary');
...
if (packageJson.private === true) failures.push('package.json must not set private=true for the public acquisition package');
...
if (packageJson.license !== 'PolyForm-Noncommercial-1.0.0') failures.push('package.json license must match LICENSE.md');
if (!packageJson.repository?.url?.includes('github.com/liufeng1976/bossai-ecommerce-ai-team-skill')) {
  failures.push('package.json repository must point to the canonical public GitHub repository');
}
if (!packageJson.homepage?.includes('github.com/liufeng1976/bossai-ecommerce-ai-team-skill')) {
  failures.push('package.json homepage must point to the canonical public GitHub repository');
}
...
for (const marker of ['source-available', 'COMMERCIAL_LICENSE.md', 'BossAI OS']) {
...
console.log('- commercial authorization marker: present');
